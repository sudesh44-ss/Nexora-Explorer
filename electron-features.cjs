...
ipcMain.handle("ai:get-index-status", async () => {
  return aiSearchService.getIndexStatus();
});

ipcMain.handle("ai:rebuild-index", async (event, targetDir) => {
  return await aiSearchService.rebuildIndex(targetDir, event.sender);
});

ipcMain.handle("ai:pause-indexing", async () => {
  return aiSearchService.pauseIndexing();
});

ipcMain.handle("ai:resume-indexing", async () => {
  return aiSearchService.resumeIndexing();
});

ipcMain.handle("ai:search", async (_event, payload = {}) => {
  const query = typeof payload === "string" ? payload : payload.query;
  const filters = payload.filters || {};
  const options = payload.options || {};
  return await aiSearchService.search(query, filters, options);
});

ipcMain.handle("ai:get-models", async () => {
  return aiSearchService.getAllModels();
});

ipcMain.handle("ai:get-active-models", async () => {
  return aiSearchService.getActiveModels();
});

ipcMain.handle("ai:set-active-model", async (_event, task, modelId) => {
  return aiSearchService.setActiveModel(task, modelId);
});

ipcMain.handle("ai:verify-model", async (_event, modelId) => {
  return aiSearchService.verifyModel(modelId);
});

ipcMain.handle("ai:get-settings", async () => {
  return aiSearchService.loadSettings();
});

ipcMain.handle("ai:save-settings", async (_event, newSettings) => {
  return aiSearchService.saveSettings(newSettings);
});

ipcMain.handle("ai:get-storage-info", async () => {
  return aiSearchService.getStorageInfo();
});

ipcMain.handle("ai:clear-cache", async () => {
  return aiSearchService.clearCache();
});

ipcMain.handle("ai:optimize-database", async () => {
  return aiSearchService.optimizeDatabase();
});

ipcMain.handle("ai:check-integrity", async () => {
  return aiSearchService.checkIntegrity();
});

ipcMain.handle("ai:repair-index", async () => {
  return aiSearchService.repairIndex();
});

ipcMain.handle("ai:clear-data", async () => {
  return aiSearchService.clearAIData();
});

ipcMain.handle("ai:get-models-status", async () => {
  return aiSearchService.getModelsStatus();
});

ipcMain.handle("ai:detect-hardware", async () => {
  return aiSearchService.getHardwareProfile();
});

ipcMain.handle("ai:select-recommended-models", async (_event, capabilities) => {
  return aiSearchService.getRecommendedModels(capabilities);
});

ipcMain.handle("ai:download-model", async (event, modelId) => {
  return await aiSearchService.downloadModel(modelId, event.sender);
});

ipcMain.handle("ai:download-recommended-pack", async (event, capabilities) => {
  return await aiSearchService.downloadRecommendedPack(capabilities, event.sender);
});

ipcMain.handle("ai:cancel-model-download", async (_event, modelId) => {
  return aiSearchService.cancelDownload(modelId);
});

ipcMain.handle("ai:uninstall-model", async (_event, modelId) => {
  return aiSearchService.uninstallModel(modelId);
});

ipcMain.handle("ai:validate-custom-model", async (_event, sourcePath) => {
  return aiSearchService.validateCustomModel(sourcePath);
});

ipcMain.handle("ai:import-custom-model", async (_event, validationResult, userOverrides) => {
  return await aiSearchService.importCustomModel(validationResult, userOverrides);
});

ipcMain.handle("ai:cancel", async () => {
  aiSearchService.pauseIndexing();
  aiSearchService.cancelDownload();
